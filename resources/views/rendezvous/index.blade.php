        ...
        <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap4">
            <div class="row">
                    ...
                    <div class="dt-buttons btn-group flex-wrap">      
                        <a href="{{url('rendezvous/pdf')}}" class="btn btn-light float-end"><i class="fa-solid fa-file-pdf" title="pdf"></i> pdf</a>
                        <a href="{{url('rendezvous/excel')}}" class="btn btn-light float-end"><i class="fa-solid fa-file-excel" title="exel"></i> excel</a> 
                        @if(!empty($chercher))
        
                        <a href="{{url('rendezvous')}}" class="btn btn-light float-end">Annuler</a>
                        @endif
                    </div></div>
                    <div class="col-sm-12 col-md-6">
                    <div id="example1_filter" class="dataTables_filter">
                        <form class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search" action="{{url('rendezvous')}}" method="GET">
    ...
    <tbody class="table-group-divider" >
        @forelse ($rendezvou as $rendezvous)
                    <tr>
                        ...
                        <td>
                            <form action="{{url('rendezvous/'.$rendezvous->num_rendezvous)}}" method="POST">
                                {{ csrf_field() }}
                                {{method_field('DELETE')}}
                                <a href="{{url('rendezvous/'.$rendezvous->num_rendezvous.'/afficher')}}" class="btn btn-primary"><i class="fa-solid fa-eye"></i></a>
...
swal({
icon:"error",
title: `Etes vous sur de vouloir supprimer ce rendez-vous`,
buttons: ["annuler","oui"],
dangerMode:true,
