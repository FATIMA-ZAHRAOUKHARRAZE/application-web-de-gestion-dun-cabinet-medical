...
<br>
            ...
            <h4 style="text-align: center">Nom&Prénom:<span>
                {{$controle->nom_patient}}</span><span> {{$controle->prenom_patient}}</span>
            </h4>
        <div id="contenu">
            @forelse ($ordonnance as $ordonnance)
            <h4><span>{{$ordonnance->nom_medicament}}</span>
                <span>{{$ordonnance->quantite_medicament}}</span>
                <span>{{$ordonnance->posologie_medicament}}</span></h4> 
                            @empty   
                            @endforelse 
</div>     
